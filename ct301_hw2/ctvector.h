...
public:
  ...
  explicit Vector(size_type count) : Vector() { // given
    if (count > 0) {
      reserve(count);
  ...
  Vector &operator=(std::initializer_list<T> list) { // given
    assign(list);
    return *this;
  }
  ...
  void assign(std::initializer_list<T> ilist) { // given (delegates)
  ...
  const T &at(size_type pos) const {
    throw std::logic_error("ct::Vector: TODO at");
  }
  T &operator[](size_type pos) {
    throw std::logic_error("ct::Vector: TODO operator[]");
  }
  ...
  size_type max_size() const noexcept { // given
  ...
  void reserve(size_type new_cap) {
    ...
    std::destroy_n(
        elements_,
        new_cap); // call destructor on each element but dosent free the memroy
  ...
  iterator insert(const_iterator pos, const T &value) { // given
  ...
  iterator insert(const_iterator pos,
                  std::initializer_list<T> il) { // given (delegates)
    return insert(pos, il.begin(), il.end());
  }

  template <typename... Args>
  iterator emplace(const_iterator pos, Args &&...args) { // given
    const size_type index = static_cast<size_type>(pos - begin());
