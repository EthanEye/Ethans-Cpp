...
std::unique_ptr<Item>
Inventory::extract(std::vector<std::unique_ptr<Item>> &from,
                   std::string_view id) {
  auto it = std::find_if(
      from.begin(), from.end(),
...
// [TODO] Walk every item, ask each one for the given attribute
// (p->attribute(key)), and keep the ones whose value equals 'value'. Return
// those borrowed pointers. This is the pattern that select_in_state /
// select_older_than also follow.
std::vector<Item *> Inventory::select_by(std::string_view key,
...
std::vector<Item *> Inventory::select_top_by_value(std::size_t n) {
  ...
  std::partial_sort(
      ...
      [](Item *a, Item *b) { return a->currentValue() > b->currentValue(); });
  all.resize(n);
  return all;
}

// [TODO] Group ALL items by the given attribute. The map key is the attribute
// value; items that lack the attribute go under the key "(none)". Return the
// map.
std::map<std::string, std::vector<Item *>>
Inventory::group_by(std::string_view key) {
  std::vector<Item *> all;
  gather(inStock_, all);
  gather(onLoan_, all);
  gather(sold_, all);
...
std::map<std::string, double> Inventory::value_by_category() const {
 ...
 std::map<std::string, double> map;
  for (auto& item  : inStock_ ) {
       auto cat = item->attribute("category");
    if (cat) {
        map[*cat] += item->currentValue();
    }
...
// InStock, then move it into inStock_. (Hint:
...
// back to InStock, move it into inStock_, and remove its LoanRecord from
// loans_. Return the rejects.
std::vector<Item *> Inventory::return_items(const std::vector<Item *> &items) {
  std::vector<Item *> rejects;
  for (auto *p : items) {
    auto uptr = extract(onLoan_, p->id());
    if (!uptr) {
      rejects.push_back(p);
      continue;
    }
    ...
    std::erase_if(loans_,
                  [&](const LoanRecord &r) { return r.itemId == p->id(); });
    uptr->setState(State::InStock);  // 
    inStock_.push_back(std::move(uptr));
  }
  return rejects;
}
...
void Inventory::print(const std::vector<Item *> &items,
...
void Inventory::print(const std::map<std::string, double> &table,
                      std::ostream &os) const {
  (void)table;
  (void)os; // remove when you implement this
}
...
void Inventory::report(std::ostream &os) const {
  int inStockCount = inStock_.size();
  int onLoanCount = onLoan_.size();
  int soldCount = sold_.size();
  int surplusCount = surplus_.size();

  // calculate in-stock value
  double inStockValue = 0.0;
  for (const auto &item : inStock_) {
    inStockValue += item->currentValue();
  }

  os << "=== Inventory Report ===\n";
  os << "InStock: " << inStockCount << "\n";
  os << "OnLoan: " << onLoanCount << "\n";
  os << "Sold: " << soldCount << "\n";
  os << "Surplus: " << surplusCount << "\n";
  os << std::fixed << std::setprecision(2);
  os << "In-stock value: " << inStockValue << "\n";
  os << "Revenue: " << revenue_ << "\n";
  ...
  for (const auto &loan : sorted) {
    os << "  " << loan.itemId << " -> " << loan.borrower << " (" << loan.when
       << ")\n";
  }

  os << "Surplus count: " << surplusCount << "\n";
}
